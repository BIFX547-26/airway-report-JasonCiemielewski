...
-   **Differential Expression**: Use the slider below to adjust the significance threshold (adjusted p-value) and see the change in the volcano plot.

```{shinylive-r}
#| standalone: true
#| viewerHeight: 600

# Explicitly list libraries to help the bundler
library(shiny)
library(ggplot2)
library(dplyr)
library(munsell) 
...
ui <- fluidPage(
  fluidRow(
    column(12,
      sliderInput("thresh", "Significance Threshold (p-adjusted):",
  ...
  fluidRow(
    column(12,
      plotOutput("volcanoPlot", width = "100%")
    )
  )
)
